...
Google Search Console raw 데이터의 쿼리 텍스트를 키워드 매핑으로 division(`DIV1/DIV2/DIV3/...`)별 분류하는 파이썬 스크립트.
...
├── gsc_division_mapping.md     # 상세 운용 가이드 (컬럼 구조, 동작 흐름, 테스트 결과)
...
- 한 셀에서 여러 division이 매칭되면 `DIVISION_ORDER = ["DIV1","DIV2","DIV3"]` 순서로 정렬 후 `", "`로 join
- 매칭 없으면 `"ETC"`
...
1. `gsc_division_keyword.csv`를 실제 사용할 키워드 셋으로 채움
   ...
   - `Division` 열: `DIV1` / `DIV2` / `DIV3` 등 라벨
